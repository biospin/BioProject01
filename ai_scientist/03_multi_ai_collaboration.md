...
2. **모델 티어링**: 저위험 작업(리뷰·테스트·문서화)은 Sonnet/Haiku로, 핵심 분석만 Opus로. `Next Agent`별로 모델을 다르게 물린다. 이 매핑은 프롬프트에 흩뿌리지 말고 하나의 검증된 설정 객체로 두는 편이 낫다. 딥리서치 하네스 분석(`docs/hyperresearchdeck.html`)은 역할별 모델 배분을 pydantic 설정(엄격 검증·frozen) 한 곳에 모아, 비용 실험을 설정 한 줄 교체로 떨어뜨린다.
