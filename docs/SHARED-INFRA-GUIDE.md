...
| BIOP02 GPU env (canonical) | git BIOP02 repo `guide/gpu_env_biop02.md` (commit `ee9f00a`) | BIOP02 spatialpatho env 정확 버전·gated 모델 접근 |
...
- **접속(정본)**: 직접접속 `@121.126.38.195` + 계정별 SSH 포트. (구 방식: bastion `61.109.239.220` 경유→`192.168.0.85` — 서버 변경으로 **폐기**.)
...
```bash
# 정본 = 직접접속(@121.126.38.195). 목적지는 localhost가 아니라 kkkim 컨테이너 IP(예시 172.18.0.5). 본인 SSH 포트 사용.
ssh -p <본인포트> -L 8899:172.18.0.5:8899 <계정>@121.126.38.195
...
| env | method | GPU | 활성화 |
|---|---|---|---|
| `velo-tf` | CRAK-Velo/UniTVelo (TF 2.13) | ✅ A6000×3 (설정 완료) | `conda activate /opt/envs/velo-tf` |
| `velo-torch` | MultiVeloVAE/MoFlow | ✅ cu121 (자체 번들) | `conda activate /opt/envs/velo-torch` |
| `celldancer` | cellDancer(참고) | CPU (격리 env) | `conda activate /opt/envs/celldancer` |
| `velo-mv` | MultiVelo/scVelo | CPU | `conda activate velo-mv` |
| `scv-preprocess` | 공통 전처리 + JupyterLab 구동 | CPU | `conda activate scv-preprocess` |
| `seqtools` | STAR·sra-tools·velocyto (raw seq 처리) | CPU | `conda activate seqtools` |

> 명명(2026-07-11): velocity env는 `velo-*` 접두(구 mv/tf/torch). 유틸은 `seqtools`로 통합. velocity 5종 전부 팀공유 `/opt/envs`, 개인 `~/miniconda3/envs`엔 scv-preprocess만(협업서버). `-n <env>` 자동 해소. ※ /opt 컨테이너 간 공유 마운트 여부는 팀원 `ls /opt/envs`로 확인(미공유 시 /workspace로 이전).

- `velo-tf` GPU: `conda activate`만으로 인식(activate.d 훅이 `LD_LIBRARY_PATH` 자동 설정). `tensorflow[and-cuda]` 금지 — 상세 `env/README.md`.
- `velo-torch` GPU: cu121 wheel이 CUDA 자체 번들 → 바로 인식.
...
| 항목 | 값 |
|---|---|
| env / 경로 | `spatialpatho` / `/home/kkkim/miniconda3/envs/spatialpatho` (Python 3.13.13, 격리 env) |
| GPU 스택 | torch 2.6.0+cu124 · torchvision 0.21.0+cu124 (CUDA 12.4, cuDNN 9.1.0) — **순수 torch, TF 없음** |
| 용도 | UNI/CONCH 파운데이션 모델 WSI 병리 임베딩 + attention-MIL(CLAM) 표현형/변이 예측 |
| 활성화 | `conda activate spatialpatho` |
| 핵심 deps | timm 1.0.27 · openslide-python 1.4.3 · huggingface_hub 1.15.0 · transformers 5.12.1 · scikit-learn 1.9.0 |
| 설치 함정 | openslide-python은 C 라이브러리 별도 → `openslide-bin`(번들 휠) 같이 설치. opencv는 `opencv-python-headless==4.11.0.86`(서버 headless) |

- **canonical(BIOP02)**: BIOP02 repo `guide/gpu_env_biop02.md` (정확 버전·gated 모델 접근 포함, commit `ee9f00a`). 이 §4.2는 요약, 상세·최신은 그 문서 참조.
- ⚠️ **`spatialpatho`는 BIOP02 전용 격리 env — BIOP01 env 통합·rename 대상 아님**(손대지 말 것).
- ⚠️ **BIOP01 env와 병합 비권장**: BIOP02=cu124(순수 torch) vs BIOP01=cu121+tf-keras 조합 → 한 env 혼재 시 충돌. **프로젝트별 분리 유지**. 공유는 GPU 공용 예절(§3)만(무거운 작업 슬롯은 `#biop02-alerts` 인덱스 예약).
- GPU 공용 예절(§3)은 BIOP02에도 동일 적용.
...
> **인프라**: 자체 A6000×3(48GB) Threadripper 서버(직접접속 @121.126.38.195). 6인 개별 컨테이너 위에 **실시간 협업 JupyterLab**(단일 인스턴스 공유) 운영. GPU는 `CUDA_VISIBLE_DEVICES` 분담 + 무거운 작업 슬롯 예약으로 조율. 프레임워크별 격리 conda env(velocity 5종: TF/PyTorch/celldancer 등)로 의존성 충돌 차단.
