...
| BIOP02 GPU env (canonical) | git BIOP02 repo `guide/gpu_env_biop02.md` (commit `a968ba4`) | BIOP02 정확 버전·gated 모델 접근 |
...
| 항목 | 값 |
|---|---|
| env / 경로 | `base` / `/home/kkkim/miniconda3` (전용 named env 없음, Python 3.13.13) |
...
| 활성화 | `conda activate base` (또는 `/home/kkkim/miniconda3/bin/python3`) |
| 핵심 deps | timm 1.0.27 · openslide-python 1.4.3 · huggingface_hub 1.15.0 · transformers 5.12.1 · scikit-learn 1.9.0 |

- **canonical(BIOP02)**: BIOP02 repo `guide/gpu_env_biop02.md` (정확 버전·gated 모델 접근 포함, commit `a968ba4`). 이 §4.2는 요약, 상세·최신은 그 문서 참조.
