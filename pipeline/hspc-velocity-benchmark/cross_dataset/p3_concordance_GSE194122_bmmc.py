...
transcription rate **α는 robust(cross-method ρ≈0.88)**"가 **같은 조직축(human bone marrow
hematopoiesis, 독립 donor/site)**에서 재현되나? — HSPC 최근접 same-tissue replication.

E18 스크립트를 미러하되, human↔human이라 **case/ortholog 매핑 불필요**(HSPC와 gene SYMBOL 축 직접 겹침).
방어적으로 양쪽 uppercase 정규화만 적용(human-human엔 무해).

두 축:
  A. Within-BMMC cross-method (동일 데이터셋):
     - α robust leg: floor·MV·VAE 사이 fit α pairwise Spearman
     - lag fragile leg: chromatin-aware 2개(MV, VAE) 사이 lag *크기* rank Spearman
  B. Cross-dataset HSPC↔BMMC (같은 human hematopoiesis, 직접 gene 매칭):
     - MV lag-크기 rank (HSPC↔BMMC) vs α rank (HSPC↔BMMC)

lag 정의 (p3_concordance.py / E18과 동일):
...
실행: conda run -n scv-preprocess python cross_dataset/p3_concordance_GSE194122_bmmc.py
출력: results/concordance_GSE194122_bmmc.md
"""
...
def main():
    ...
    L = ["# P3 concordance — GSE194122 human BMMC replication of HSPC lag/α finding", "",
         "> 질문: HSPC 결론 **\"lag은 method-robust하지 않고 α는 robust(cross-method ρ≈0.88)\"**가 "
         "**같은 조직축(human BMMC hematopoiesis, 독립 donor09/site4)**에서 재현되나? — "
         "same-tissue 최근접 replication.", "",
         "## 실행된 method arm", ""]
    ...
    # ── A. Within-BMMC cross-method ──
    L += ["## A. Within-BMMC cross-method concordance", "",
          "동일 데이터셋·동일 gene축.", ""]
    ...
    # ── B. Cross-dataset HSPC ↔ BMMC (직접 gene 매칭, human↔human) ──
    L += ["## B. Cross-dataset HSPC ↔ BMMC (human↔human, 직접 gene 매칭)", "",
          "> 둘 다 human hematopoiesis → gene SYMBOL 축 직접 겹침(case/ortholog 매핑 불필요, "
    ...
    if h_mv_u is not None and b_mv_u is not None:
        ...
        L.append(f"- shared gene (HSPC × BMMC MultiVelo): **{up_shared}** "
                 "(same-tissue human → 교집합 큼).")
        h_lag = (h_mv_u["fit_t_sw2"] - h_mv_u["fit_t_sw1"]).abs()
        e_lag = (b_mv_u["fit_t_sw2"] - b_mv_u["fit_t_sw1"]).abs()
        line, b_lag_rho, n = rho_line(h_lag, e_lag, "MV lag 크기 rank: HSPC × BMMC")
        L.append(line)
        ...
        if h_fl_u is not None and b_fl_u is not None:
            line, r, n = rho_line(h_fl_u["fit_alpha"], b_fl_u["fit_alpha"],
                                  "floor α rank: HSPC × BMMC (sanity)")
            L.append(line)
    else:
        L.append("- HSPC 또는 BMMC MultiVelo csv 없음 → cross-dataset 비교 skip.")
    L.append("")
    ...
    L.append(f"- Within-BMMC α (cross-method) Spearman: {[f'{r:+.2f}' for r in a_rhos] or 'N/A'} "
             f"(중앙값 {_fmt(a_alpha_med)})")
    L.append(f"- Within-BMMC lag 크기 rank (MV×VAE): {_fmt(lag_rho)}")
    L.append(f"- Cross-dataset HSPC↔BMMC: lag 크기 rank {_fmt(b_lag_rho)} vs α rank {_fmt(b_alpha_rho)}")
    L.append("")
    ...
    L += ["",
          ...
          "- 단일 donor(site4/donor9, ~4.3k cells) replication 1건 — 강한 일반화 주장 금지. "
          "HSPC + human_brain + E18 + BMMC 축의 일관성으로만 서사.",
          "- concordance는 *전역* per-gene fit rank (within-lineage 아님). BMMC lineage(제공 cell_type)는 "
          "load-bearing 아님.",
          "- RNA는 우리가 donor09 GEX BAM에 velocyto run으로 복구(spliced/unspliced), ATAC는 processed "
          "h5ad peak matrix의 gencode-proximity gene 집계 — HSPC의 mv.aggregate_peaks_10x와 구현 다름(문서화). "
...
def _verdict(a_alpha, lag_within, b_lag, b_alpha):
    ...
    if alpha_robust and (lag_fragile_within or lag_fragile_cross):
        ...
        if lag_within is not None:
            legs.append(f"within-BMMC MV×VAE lag {lag_within:+.2f}")
        ...
        return ("재현 YES — 'lag-fragile / α-robust' 패턴 재현",
                ...
                f"lag는 약함({', '.join(legs)}). HSPC same-tissue 서사 일치.")
