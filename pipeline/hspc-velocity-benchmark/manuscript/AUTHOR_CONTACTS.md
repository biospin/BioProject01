...
**OpenReview 가입은 기관 주소인 `siegfried@yonsei.ac.kr`로 하는 쪽이 유리하다** — 공용 도메인
가입은 심사 큐에 걸려 최대 2주가 걸린다. 다만 제출 폼에 자리표시로 들어간 주소가
`jaemyunlyu@gmail.com`이므로, **프로필에 두 주소를 모두 넣어야** 이 논문이 계정에 붙는다.
...
이해충돌 정보도 그분들 프로필에서 읽힌다. 류재면 님은 이메일 자리표시로 들어갔고,
**2026-09-11 기준 아직 프로필로 연결되지 않았다.** 심사자 배정이 9월 14일에 끝나므로
그 전에 프로필이 만들어져야 이해충돌 정보가 반영된다.
