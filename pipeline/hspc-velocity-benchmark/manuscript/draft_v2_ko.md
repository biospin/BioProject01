...
유전자별 kinetic 모수는 velocity가 주로 쓰이는 대상이 아니므로, 감사를 한 층 위로 넓혀 모든 방법이 임베딩 투영 이전에 산출하는 세포×유전자 velocity 행렬을 보았다. 다섯 arm 모두에서 쓸 수 있는 21,878개 세포와 354개 유전자에서(spliced velocity만, 어느 arm에서든 velocity가 결측이거나 상수인 유전자는 제외), 방법 간 세포별 코사인 유사도는 모든 multiome 쌍에서 세포 셔플 귀무 이하였고(초과분 −0.263 ~ +0.003), (세포,유전자) 단위 부호 일치는 49.7~58.3%였다. 이 숫자를 해석 가능하게 만드는 대조군이 둘이다. 첫째는 재현성 천장이다. MultiVelo를 재표집한 세포에 다시 fitting하면 자기 행렬을 평균 중심화 코사인 +0.872로 재현하며(재적합 6회, 범위 +0.826 ~ +0.887; 부호 일치 78.6%), 따라서 이 지표는 일치가 있는 곳에서 일치를 검출하고 방법 간 값은 측정 잡음이 아니라 실제 불일치다. 둘째는 시간차에 적용했던 것과 같은 인과 대조군이다. ATAC 유전자 라벨을 뒤섞어도 MultiVelo 행렬은 재적합이 움직인 만큼밖에 움직이지 않아(+0.838, 재적합 범위 안), chromatin이 행렬에 기여하는 몫이 실행 간 변동 안에 놓인다. 이 비교 전체에서 가장 강한 일치는 두 multiome 방법 사이가 아니라 MultiVelo와 RNA 전용 scVelo floor 사이였다(+0.583). 다만 이는 계열 전체의 성질이 아니다. CRAK-Velo, MoFlow, MultiVeloVAE의 대응값은 각각 +0.260, −0.004, −0.292였다. 그리고 MultiVelo와 MultiVeloVAE는 같은 세포에 체계적으로 반대 방향을 부여했다(−0.500). 인과적 해석은 재적합 대조군을 가진 유일한 arm인 MultiVelo에 한정한다. MoFlow는 확률적 심층 모형이어서 원본 대 셔플 값(+0.113)을 같은 seed 재실행 없이는 실행 간 변동과 분리할 수 없다. 이 분석은 fitting 결과를 읽기 전에 반증 기준을 봉인해 사전등록했고 사전등록한 두 대조 모두 실패했으므로(Additional file 12), 유전자별 결론이 행렬로 확장된다. 이는 일반 벤치마크와도 나란히 놓인다. 그들은 RNA 전용 환경에서 전이 벡터의 방법 간 일치가 낮다고 보고한다(비교한 12개 방법 전부 A1<0.3) [12].
...
**Additional file 12: Table S10.** 세포 수준 velocity 행렬 감사 — HSPC의 방법 쌍별 세포별·유전자별 코사인 유사도와 세포 셔플 귀무, 평균 중심화 값, (세포,유전자) 부호 일치, ATAC-shuffle 대조, 재적합 재현성 천장, 그리고 외부 4개 시스템의 같은 쌍 표와 paired 기준선 대조, 봉인된 사전등록(본 프로토콜과 외부 재현 부록)과 채점 결과. (.csv, .pdf)
