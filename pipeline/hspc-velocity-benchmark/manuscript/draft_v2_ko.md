...
그 자리를 대신하는 진술은 의도적으로 약하다. 짝지은 차이는 작고 고르지 않으며(Δ = A − B가 +0.081, +0.004, +0.063이고 중앙값 +0.063으로 천장 +0.872의 7% 수준), 셋 중 하나는 방향의 부호를 빼면 사실상 null이라 셋이 하나의 일관된 기전으로 읽히지 않는다. 크기 또한 같은 지표에서 method 선택이 만들어 내는 불일치(방법 쌍 간 평균 중심화 코사인 −0.530 ~ +0.131)보다 한참 아래다. 이 설계로는 양성 인과 주장도 할 수 없다. MultiVelo에 대해 같은 세포·같은 설정의 재실행 귀무가 아직 확립되지 않아, 짝지은 Δ에는 크로마틴의 기여와 통상적인 재적합 잡음이 섞여 있고 +0.063 중 어느 쪽이 얼마인지 나눌 수 없다. 여기서 말할 수 있는 것은 크로마틴을 파괴하면 이 행렬이 움직이는 곳에서도 조금만 움직인다는 것이며, 이는 크로마틴이 기여한다는 증명도 아니고 무력하다고 부를 근거도 아니다. 이 비교들을 평균 중심화 코사인으로 보고하는 이유는 원척도 값이 모든 세포에 공통인 방향에 부분적으로 좌우되기 때문이다(평균 벡터가 행 노름 제곱의 12.9~37.4%를 차지하며 arm마다 다르다). 중심화는 사후 진단이고 봉인된 지표 목록에 없으며, 위의 사전등록 판정은 원척도 지표에 기댄다. 무게가 실린 한계가 하나 더 있다. 방법 간 값을 arm 내부 불안정이 아니라 진짜 불일치로 읽는 것은 그 대조군을 가진 MultiVelo에서만 허용된다. 0에 가장 가까운 세 쌍은 모두 MoFlow가 낀 쌍인데, MoFlow는 확률적 심층 모형이고 동일 설정 재실행 안정성이 확립된 적이 없다(자기 원본 대 셔플 값 +0.113도 같은 이유로 해석 불가). 그 쌍들에서는 불일치와 불안정이 분리되지 않는다.
