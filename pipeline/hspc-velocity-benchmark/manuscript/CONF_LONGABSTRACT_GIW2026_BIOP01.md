...
- 분량(2026-08-10 트림): 본문 산문(Background~Conclusion) 1125→1101단어(목표 1050~1150 내). 중복 서사만 압축, 수치·claim·caveat 100% 보존(numeric-token diff 손실 0). 참고문헌 포함 전체는 1495→~1471단어. 2p 적합은 단어수보다 그림 2개+참고문헌 10개가 좌우 — 조판에서 넘치면 산문 추가 압축이 아니라 그림 폭·배치로 조정.
  - 실측 정정(2026-08-14, 류재면 검토 #11894 반영): 참고문헌은 11개가 아니라 **10개**(`^\[N\]` 카운트). 그림 실물도 확인 — `fig01_p2_concordance.png` 1935×563px(메타 dpi 130), `fig07_reliability_map.png` 4539×2812px(300dpi). fig01은 메타 dpi가 300이 아니므로 **본문 폭 6.5인치 이하로 배치**해야 실효 해상도가 300dpi 근처로 유지된다(1935px ÷ 6.5in ≈ 298dpi). 조판 시 확인할 것.
════════════════════════════════════════════════════════════════════════
-->
