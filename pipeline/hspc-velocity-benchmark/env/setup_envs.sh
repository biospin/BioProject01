...
#   인자 없으면 4개 모두. 예: setup_envs.sh mv   (mv만)
#
# 권장: conda 대신 mamba(빠름). CONDA=mamba bash setup_envs.sh
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
CONDA="${CONDA:-conda}"
if [ $# -eq 0 ]; then ENVS=(scv-preprocess mv torch tf); else ENVS=("$@"); fi
...
echo ""
echo "다음:"
echo "  1) 각 env에서 import 확인:  conda run -n mv python -c 'import multivelo, scvelo; print(\"mv ok\")'"
