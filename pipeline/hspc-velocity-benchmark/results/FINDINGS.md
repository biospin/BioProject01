...
> 최종 갱신: **2026-07-06** (§7 cross-dataset replication: `concordance_human_brain.md`, `concordance_e18_mouse_brain.md`, **`concordance_GSE194122_bmmc.md`(same-tissue human BMMC 추가)**).
...
| **cross-dataset replication** (human_brain + E18 mouse brain + human BMMC) | ✅ | `concordance_human_brain.md` + `concordance_e18_mouse_brain.md` + `concordance_GSE194122_bmmc.md` |
...
이 'α > lag' 순서는 **세 외부 데이터셋(성인 human_brain·태아 E18 mouse brain·same-tissue human BMMC)에서도 보존된다**(§7). 이는 HSPC 한 데이터셋에서만 나온 우연이 아니다. cross-dataset α는 조직이 멀수록 순서대로 낮아지지만(단조 감소; BMMC +0.55 > human_brain +0.475 > E18 +0.32) lag은 어디서도 무신호다(+0.05~+0.19).
...
HSPC 한 데이터셋에서만 나온 산물이 아님을 세 외부 multiome으로 검정한다(same-tissue human BMMC 포함). **핵심은 절대값이 아니라 α > lag 순서의 보존이다.**
...
- **(C) human BMMC (GSE194122, donor09/site4, HSPC와 같은 조혈 축 — same-tissue 최근접 재현, 우리가 GEX BAM velocyto로 spliced/unspliced 복구)** `concordance_GSE194122_bmmc.md`:
  ...
  - **cross-dataset HSPC↔BMMC**(human↔human, SYMBOL 직접 매칭 shared 88): α rank **+0.550**(p=2.9e-8) vs lag 크기 rank **+0.052**(p=0.63), floor α sanity +0.558. → same-tissue라 셋 중 **cross-dataset α가 가장 강함**(0.55).
> 판정: **within-dataset에선 α-robust/lag-fragile 순서가 강하게 재현**(BMMC within α 0.85 ≫ lag −0.09; E18 within α 0.81 ≫ lag 0.06)되고, **cross-dataset α는 조직 거리에 단조 감쇠하나 lag은 어디서도 무신호**: same-tissue human BMMC α **+0.55** > 성인 human_brain **+0.475** > cross-species E18 **+0.32**, 반면 lag은 세 축 모두 +0.05~+0.19 잡음이다. 즉 "α가 종을 넘어 보존된다"는 강한 주장이 아니라 **"어느 데이터셋에서 재도, α가 lag보다 method·데이터셋에 robust하다"는 순서 진술**이 4개 축(HSPC + human_brain + E18 + BMMC)에서 일관된다.
> ⚠️ 정직 caveat: lag-fragile leg는 within-dataset에서 **단일 method 쌍(MV×VAE)에만 근거한다**(BMMC·E18 모두 MoFlow/CRAK 미실행 → HSPC 3쌍보다 얇음). 세 replication 모두 각 1 donor/샘플 — 강한 일반화 금지, 네 축 일관성으로만 서사한다. BMMC ATAC는 processed peak matrix의 gencode-proximity 집계다(HSPC의 mv.aggregate_peaks_10x와 구현 상이 → cross rank에 보수적 잡음).
...
> Last updated: **2026-07-06** (§7 cross-dataset replication: `concordance_human_brain.md`, `concordance_e18_mouse_brain.md`, **`concordance_GSE194122_bmmc.md` (same-tissue human BMMC added)**).
...
This 'α > lag' ordering is **preserved across three external datasets (adult human_brain · fetal E18 mouse brain · same-tissue human BMMC)** as well (§7). This is not a coincidence arising from the single HSPC dataset alone. Cross-dataset α is monotone with tissue distance (BMMC +0.55 > human_brain +0.475 > E18 +0.32), whereas lag is signal-free everywhere (+0.05~+0.19).
...
Tested across three external multiomes (including same-tissue human BMMC) that this is not a coincidence arising from one HSPC dataset alone. **The key is not the absolute value but the preservation of the α > lag ordering.**
...
- **(C) human BMMC (GSE194122, donor09/site4, same hematopoietic axis as HSPC — same-tissue nearest replication, we recovered spliced/unspliced via GEX BAM velocyto)** `concordance_GSE194122_bmmc.md`:
  ...
  - **cross-dataset HSPC↔BMMC** (human↔human, direct SYMBOL matching, shared 88): α rank **+0.550** (p=2.9e-8) vs lag magnitude rank **+0.052** (p=0.63), floor α sanity +0.558. → being same-tissue, **cross-dataset α is the strongest of the three** (0.55).
> Verdict: **within-dataset the α-robust/lag-fragile ordering reproduces strongly** (BMMC within α 0.85 ≫ lag −0.09; E18 within α 0.81 ≫ lag 0.06), and **cross-dataset α decays monotonically with tissue distance while lag is signal-free everywhere**: same-tissue human BMMC α **+0.55** > adult human_brain **+0.475** > cross-species E18 **+0.32**, whereas lag is +0.05~+0.19 noise on all three axes. That is, not the strong claim "α is preserved across species" but rather the **ordering statement "in whatever dataset, α is more robust to method·dataset than lag"**, consistent across 4 axes (HSPC + human_brain + E18 + BMMC).
> ⚠️ honest caveat: the lag-fragile leg rests within-dataset on **only a single method pair (MV×VAE)** (both BMMC·E18 have MoFlow/CRAK unrun → thinner than HSPC's 3 pairs). All three replications are 1 donor/sample each — no strong generalization; the narrative rests only on the consistency of the four axes. BMMC ATAC is a gencode-proximity aggregation of the processed peak matrix (implementation differs from HSPC's mv.aggregate_peaks_10x → conservative noise on the cross rank).
