...
| **cross-dataset replication** (human_brain + E18 mouse brain + human BMMC + macrophage + mouse gastrulation) | ✅ | `concordance_human_brain.md` + `concordance_e18_mouse_brain.md` + `concordance_GSE194122_bmmc.md` + `concordance_macrophage.md` + `prereg_gse205117_scorecard.md`(사전등록 6/0) |
...
일치도가 높지 않으며(H1 실패 — **CRAK-비의존 clean headline**: 3-method magnitude concordance |ρ|≤0.08 {mv×moflow −0.04, mv×mvvae −0.01, moflow×mvvae +0.08} + 2-method sign-agreement 54.6%≈chance(방향 미정 lag=0 76개 제외; 미제외 np.sign 규약이면 48%). permutation-FDR **agreement-set 0/598은 부호 가변 method 3개가 필요해 CRAK 포함 시에만 정의된다 → CRAK에 의존하므로 대표 결과에서 빼고 CRAK 민감도 분석(보조)으로 다룬다**; 2026-07-05 `clean_concordance_gate.md`), 음성대조는 한 method(MultiVelo)의 lag가
...
- **gene 수준 방향 불일치**: moflow×multivelovae sign-agreement은 방향 미정(lag=0) 76개를 제외하면 **54.6%**(n=560, 이항 p=0.031)로 우연에 가깝다. 0을 제3범주로 취급하는 np.sign 규약에서는 48%로 읽힌다 — 처리 방식 명시가 필요하다(`CORRECTION_sign_agreement_zero_handling.md`).
...
- **⚠️ clean-gate 정정(2026-07-05, `clean_concordance_gate.md`)**: (B)의 0/598은 부호 가변 method 3개(moflow, **crakvelo**, mvvae)를 요구한다. MultiVelo lag은 구조적 100% 양수(switch-time 단조정렬)라 *sign*-consistency 검정에 넣을 수 없고(null 오지정), 따라서 clean 부호 가변 method는 {moflow, mvvae} 2개뿐 → 2-method sign FDR은 min p_perm≈0.50으로 **원리적 power-bound(신호 무관 0)**. 즉 **0/598은 본질적으로 CRAK-의존** → **CRAK 민감도 arm으로 강등**. **CRAK-비의존 clean headline은 magnitude concordance**(clean 3-way |ρ|≤0.08, verify-gate `p3_concordance.py`가 재계산) + 2-method sign-agreement 54.6%(미정 제외; np.sign 규약 48%). MultiVelo는 sign이 아니라 magnitude/rank 검정에만 참여(lag 크기는 gene간 변이 有). *reframe이지 method-swap이 아님* — {mv,moflow,mvvae}로 sign 검정 재실행 금지(상수-부호 오류 재도입).
...
HSPC 한 데이터셋에서만 나온 산물이 아님을 **다섯 외부 multiome**으로 검정한다(same-tissue human BMMC·HSPC-직계 macrophage·발생 mouse gastrulation 포함). **핵심은 절대값이 아니라 α > lag 순서의 보존이다.** 다섯 번째(gastrulation)는 **사전등록 6예측 채점**으로 검정했다(fit 도착 전 봉인, §7-E).
...
- **(E) mouse gastrulation (GSE205117, E7.5–8.75 10x Multiome, 발생 아틀라스 — priming 극대 조직; GEX=STARsolo Velocyto raw, ATAC=GEO fragments 자체 peak→gene 집계, 10779 cells)** `prereg_gse205117_scorecard.md`. **이 데이터셋만 fit 도착 전 6개 예측을 봉인(사전등록)하고 사후구제 없이 채점**(paired bootstrap B=10000, seed=20260707):
  ...
  - **cross-dataset HSPC↔gastrulation**(mouse→UPPER 매핑 shared 111): α rank **+0.415**(95%CI [+0.244,+0.561]) vs lag 크기 rank **+0.028**(95%CI [−0.165,+0.224]). per-gene 불일치 lag **0.294**(MV×MoFlow c-s lag, HSPC 원정의·부호 유지, n=968) > α **0.052**.
  - **봉인 6예측 채점: 6 PASS / 0 FAIL** — priming이 극대인 발생 조직에서도 α-robust/lag-fragile이 사전등록 임계를 그대로 통과. 예측5(per-gene lag)는 **봉인된 원정의(MV×MoFlow)로 채점**(2026-07-15 MoFlow arm cross-dataset 배선 완료 → 초기 MV×VAE 치환 해소, 사후 선택 없음). 채점기 결정론 재계산 통과(scorecard byte-identical).
> 판정: **within-dataset에선 α-robust/lag-fragile 순서가 강하게 재현**(gastrulation within α 0.927 ≫ lag −0.03; macrophage within α 0.865 ≫ lag 0.07; BMMC within α 0.85 ≫ lag −0.09; E18 within α 0.81 ≫ lag 0.06)되고, **cross-dataset α는 조직 거리에 단조 감쇠하나 lag은 어디서도 무신호**: HSPC-직계 macrophage α **+0.643** > same-tissue human BMMC **+0.55** > 성인 human_brain **+0.475** > 발생 gastrulation **+0.415** > cross-species E18 **+0.32**, 반면 lag은 다섯 축 모두 +0.03~+0.19 잡음이다. 즉 "α가 종을 넘어 보존된다"는 강한 주장이 아니라 **"어느 데이터셋에서 재도, α가 lag보다 method·데이터셋에 robust하다"는 순서 진술**이 6개 축(HSPC + human_brain + E18 + BMMC + macrophage + gastrulation)에서 일관된다. **다섯 번째(gastrulation)는 이 순서를 사전등록으로 검정해 6/0 통과**(§7-E) — 사후적 확증이 아니라 봉인된 예측이라 서사의 confirmatory 강도가 높다.
> ⚠️ 정직 caveat: lag-fragile leg는 within-dataset에서 대체로 **단일 method 쌍(MV×VAE)에 근거한다**(BMMC·E18·macrophage는 MoFlow/CRAK 미실행 → HSPC 3쌍보다 얇음; **gastrulation만 MoFlow 배선되어 예측5 per-gene lag이 봉인 원정의 MV×MoFlow로 채점됨**). 다섯 replication 모두 각 1 donor/샘플 — 강한 일반화 금지, 여섯 축 일관성으로만 서사한다. BMMC ATAC는 processed peak matrix의 gencode-proximity 집계, gastrulation ATAC는 GEO fragments의 gene body ±10kb 자체 집계다(둘 다 HSPC의 mv.aggregate_peaks_10x와 구현 상이 → cross rank에 보수적 잡음). macrophage·gastrulation p·CI는 canonical numpy(B=10000, seed=20260707) 확정.
...
- **freed-nuisance 게이트 완주(2026-07-10, interior 258)**: β/γ/α_c/rescale/scale_cc를 재최적화해도 **해리(α≫lag)는 생존** — `ratio_freed` median **2.49×**, α still stiffer **77.03%**(n=148). → "nuisance 고정으로 lag을 sloppy하게 만든 아티팩트 아니냐"는 반론 **방어됨**. **단 정직 caveat: β/γ 자유화 시 α 곡률이 fixed 대비 median 0.19×로 붕괴**(alpha peak collapses) — fixed-nuisance의 3.53×/94.57%는 **상한**, freed의 2.49×/77%가 **보수적 하한**. 주장(α가 lag보다 stiff)은 두 게이트 모두에서 성립하되 freed 기준으로 서술한다. (`results/profile_likelihood_freed.csv`)
...
methods (H1 failure — **CRAK-independent clean headline**: 3-method magnitude concordance |ρ|≤0.08 {mv×moflow −0.04, mv×mvvae −0.01, moflow×mvvae +0.08} + 2-method sign-agreement 54.6%≈chance(방향 미정 lag=0 76개 제외; 미제외 np.sign 규약이면 48%). permutation-FDR **the agreement-set 0/598 requires 3 sign-variable methods, so it is defined only when CRAK is included → demoted to a CRAK sensitivity arm**; 2026-07-05 `clean_concordance_gate.md`), and the negative control showed that one method's (MultiVelo) lag
...
- **gene-level direction disagreement**: moflow×multivelovae sign-agreement **54.6%** (undetermined lag=0 excluded; 48% under the np.sign convention).
