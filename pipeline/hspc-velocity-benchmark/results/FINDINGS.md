...
이 'α > lag' 순서는 **세 외부 데이터셋(성인 human_brain·태아 E18 mouse brain·same-tissue human BMMC)에서도 보존**(§7) — HSPC 단일 데이터의 우연이 아니다. cross-dataset α는 조직 거리에 단조(BMMC +0.55 > human_brain +0.475 > E18 +0.32)이나 lag은 어디서도 무신호(+0.05~+0.19).
게다가 이 순서는 **MultiVelo 자기 목적함수의 기하학적 성질로 *입증* 승격**(§8): profile-likelihood로 α는 stiff(식별 가능), lag은 sloppy + boundary-limited 방향임을 보였다(gene별 κ_alpha/κ_lag median **3.53×**, α가 더 stiff한 gene **94.57%**). 즉 lag이 재현 안 되는 것은 잡음이 아니라 목적함수가 lag 방향으로 애초에 덜 정보적이기 때문 — 관찰이 메커니즘으로 뒷받침됨.
...
HSPC 한 데이터셋의 우연이 아님을 세 외부 multiome으로 검정(same-tissue human BMMC 포함). **핵심은 절대값이 아니라 α > lag 순서의 보존.**
...
- **(C) human BMMC (GSE194122, donor09/site4, HSPC와 같은 조혈 축 — same-tissue 최근접 재현, 우리가 GEX BAM velocyto로 spliced/unspliced 복구)** `concordance_GSE194122_bmmc.md`:
  - **within-BMMC cross-method α**(floor×MV×VAE, 같은 gene축): Spearman +0.82 / +0.85 / **+0.91**(중앙값 **+0.851**) — α-robust leg가 **같은 조직에서 가장 강하게 재현**(HSPC ρ=0.88과 정합).
  - **within-BMMC lag 크기 rank**(MV×VAE): **−0.088**(p=0.15) — lag-fragile leg 재현(HSPC MV×VAE −0.01, E18 +0.06과 정합).
...
> 판정: **within-dataset에선 α-robust/lag-fragile 순서가 강하게 재현**(BMMC within α 0.85 ≫ lag −0.09; E18 within α 0.81 ≫ lag 0.06)되고, **cross-dataset α는 조직 거리에 단조 감쇠하나 lag은 어디서도 무신호**: same-tissue human BMMC α **+0.55** > 성인 human_brain **+0.475** > cross-species E18 **+0.32**, 반면 lag은 세 축 모두 +0.05~+0.19 잡음. 즉 "α가 종을 넘어 보존된다"는 강한 주장이 아니라 **"어느 데이터셋에서 재도, α가 lag보다 method·데이터셋에 robust하다"는 순서 진술**이 4개 축(HSPC + human_brain + E18 + BMMC)에서 일관.
> ⚠️ 정직 caveat: lag-fragile leg는 within-dataset에서 **단일 method 쌍(MV×VAE)에만 근거**(BMMC·E18 모두 MoFlow/CRAK 미실행 → HSPC 3쌍보다 얇음). 세 replication 모두 각 1 donor/샘플 — 강한 일반화 금지, 네 축 일관성으로만 서사. BMMC ATAC는 processed peak matrix의 gencode-proximity 집계(HSPC의 mv.aggregate_peaks_10x와 구현 상이 → cross rank에 보수적 noise).

### 8. profile-likelihood 실질 식별성 — 'lag-fragile / α-robust'를 목적함수 성질로 입증 (2026-07-09)
cross-method/cross-dataset의 관찰(§1·§6·§7)이 *왜* 생기는지를 MultiVelo **자기 목적함수** 위에서 검정. fixed-nuisance profile likelihood를 α 방향과 lag(=t_sw2−t_sw1) 방향으로 각각 스캔(latent time 재최적화, n=538 gene; `p3_profile_likelihood.py`, fit·likelihood 재현 r≈1.0).
...
- **판정**: MultiVelo 목적함수는 **lag 방향으로 실질적으로 비식별(평탄에 가깝고 boundary-limited), α 방향으로 식별 가능(sharp)**. lag이 재현 안 되는 것은 노이즈가 아니라 우도가 lag 방향으로 애초에 덜 정보적이기 때문. **상대적(practical) 비식별성**이며 완전 평탄 valley는 아님(정직 framing). dissociation·external rate validation과 정합 → 관찰이 메커니즘으로 승격.
- **스코프/한계**: MultiVelo 단일 method의 우도 기하; freed-nuisance 게이트(β/γ/α_c/rescale/scale_cc 재최적화)는 **부분(사용가능 n=1)** — main fixed-nuisance(538)가 주장 담당. (`results/profile_likelihood_identifiability.md`, `figures/fig05_profile_likelihood.png`)
