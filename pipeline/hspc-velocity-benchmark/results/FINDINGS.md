...
- **(A) human_brain (발생기 뇌 multiome, GSE162170 Trevino 2021 developing cortex, HSPC↔human_brain MultiVelo)** `concordance_human_brain.md`:
...
methods (H1 failure — **CRAK-independent clean headline**: the lag reproduces weakly at best in magnitude (magnitude-convention strongest pair MultiVelo×MultiVeloVAE **+0.163** [95% CI +0.078–+0.244], most pairs |ρ|≤0.08; the signed-convention lag ρ {mv×moflow −0.04, mv×mvvae −0.01, moflow×mvvae +0.08} are lower than the magnitude because near-zero sign-unstable lags cancel under signing) + 2-method sign-agreement 54.6%≈chance(방향 미정 lag=0 76개 제외; 미제외 np.sign 규약이면 48%). permutation-FDR **the agreement-set 0/598 requires 3 sign-variable methods, so it is defined only when CRAK is included → demoted to a CRAK sensitivity arm**; 2026-07-05 `clean_concordance_gate.md`), and the negative control showed that one method's (MultiVelo) lag
...
- **(A) human_brain (fetal brain multiome, GSE162170 Trevino 2021 developing cortex, HSPC↔human_brain MultiVelo)** `concordance_human_brain.md`:
