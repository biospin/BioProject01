...
- **(E) mouse gastrulation (GSE205117, E7.5–8.75 10x Multiome, 발생 아틀라스 — priming 극대 조직; GEX=STARsolo Velocyto raw, ATAC=GEO fragments 자체 peak→gene 집계, 10779 cells)** `prereg_gse205117_scorecard.md`. **이 데이터셋만 fit 도착 전 6개 예측을 봉인(사전등록)하고 사후구제 없이 채점**(paired bootstrap B=10000, seed=20260707):
  ...
  - **cross-dataset HSPC↔gastrulation**(mouse→UPPER 매핑 shared 111): α rank **+0.415**(95%CI [+0.244,+0.561]) vs lag 크기 rank **+0.028**(95%CI [−0.165,+0.224]). per-gene 불일치 lag **0.307** > α **0.052**.
  - **봉인 6예측 채점: 6 PASS / 0 FAIL** — priming이 극대인 발생 조직에서도 α-robust/lag-fragile이 사전등록 임계를 그대로 통과. ⚠️ 예측5(per-gene lag)는 MoFlow arm cross-dataset 미배선이라 **R4에 사전선언한 MV-vs-VAE 치환**으로 채점(사후 선택 아님). 채점기 결정론 재계산 통과(scorecard byte-identical).
...
> ⚠️ 정직 caveat: lag-fragile leg는 within-dataset에서 **단일 method 쌍(MV×VAE)에만 근거한다**(BMMC·E18·macrophage·gastrulation 모두 MoFlow/CRAK 미실행 → HSPC 3쌍보다 얇음). 다섯 replication 모두 각 1 donor/샘플 — 강한 일반화 금지, 여섯 축 일관성으로만 서사한다. BMMC ATAC는 processed peak matrix의 gencode-proximity 집계, gastrulation ATAC는 GEO fragments의 gene body ±10kb 자체 집계다(둘 다 HSPC의 mv.aggregate_peaks_10x와 구현 상이 → cross rank에 보수적 잡음). macrophage·gastrulation p·CI는 canonical numpy(B=10000, seed=20260707) 확정.
