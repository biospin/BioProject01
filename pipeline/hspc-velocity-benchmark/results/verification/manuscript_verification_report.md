...
- git HEAD = `abaee91` | 위험 Tier = **3 (투고·공개)** | correction cap = 2
...
| 재계산 게이트 p3_concordance.py | **PASS_WITH_NOTE** | 0 | 재계산 실행 완료(rc=0) — 커밋값과 diff 0은 별도 대조 필요(이 게이트는 실행 성공만 확인) |
| 재계산 게이트 p3_crossdataset_concordance.py | **PASS_WITH_NOTE** | 0 | 재계산 실행 완료(rc=0) — 커밋값과 diff 0은 별도 대조 필요(이 게이트는 실행 성공만 확인) |
| 재계산 게이트 p3_scrambled_null.py | **PASS_WITH_NOTE** | 0 | 재계산 실행 완료(rc=0) — 커밋값과 diff 0은 별도 대조 필요(이 게이트는 실행 성공만 확인) |

## 종합: **HOLD**  (PASS 3 · PASS_WITH_NOTE 3 · HOLD 2 · FAIL 0 · SKIP 0)
