...
위 결과들은 하나로 꿰인다:
- profile-likelihood: **α가 가장 stiff**(κ +7.98) · α_c +7.32 · β +4.86 · **γ 가장 sloppy**(+1.72). ⚠️ α_c는 *한 번의 fit 안에서는* 두 번째로 뾰족하므로 "α만 stiff"라고 쓰면 우리 자신의 곡률값과 어긋난다. 정확히는 **한 fit 안의 곡률(α_c 높음)과 method를 바꿨을 때의 안정성(α_c cross-method ρ=0.29로 fragile)이 서로 다른 축**이고, α만이 두 축을 모두 통과한다.
