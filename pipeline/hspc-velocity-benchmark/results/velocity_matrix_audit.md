...
**세포 수준 velocity 행렬은 method 사이에서 재현되지 않는다.** 같은 method를 재적합하면 잘 재현되는데(중심화 코사인 +0.87), 서로 다른 multiome method 사이에서는 0 또는 음수다. 그나마 실질적인 method 간 일치는 크로마틴을 안 쓰는 **RNA-only 기준선과의 쌍**에서 나오지만, 이것도 **MultiVelo에서만 뚜렷하고**(+0.583) multiome 계열 전체의 성질은 아니다.

> ⚠️ **2026-07-20 정정.** 이 문서가 처음 쓴 "크로마틴을 셔플해도 재적합 잡음만큼밖에 안 움직인다(+0.838)"와 거기서 끌어낸 **"크로마틴은 인과적으로 무력하다"는 결론을 철회한다.** 그 비교는 천장을 재표집 15,315 cell에서, 셔플을 전량 21,878 cell에서 계산해 세포집합이 어긋나 있었다. 같은 재표집 세포집합 S_b 위에서 다시 맞춰 비교하면 셔플 값은 온전한 재적합 범위 아래로 3/3 내려간다(+0.784·+0.813·+0.810 대 +0.826~+0.887, CI 3/3 비중첩). 상세 = [`velocity_matrix_paired_shuffle.md`](velocity_matrix_paired_shuffle.md). 다만 **반대 방향의 양성 단정도 하지 않는다** — 같은 세포·같은 설정의 run-to-run 귀무가 아직 없어 짝지은 Δ(+0.081·+0.004·+0.063)는 크로마틴 기여와 재적합 잡음이 섞인 값이다. 아래 §1 B행·§4·§5-2는 이 정정에 맞춰 고쳐 뒀고, 원래 수치는 기록으로 남긴다.
...
B행의 "안 무너진다"는 그대로 둔다(짝맞춘 대조에서도 셔플 행렬은 붕괴하지 않는다). 다만 여기 적힌 +0.732와 아래 §4의 +0.838은 **세포집합이 어긋난 비교**이므로, 크기 판단은 짝맞춘 [`velocity_matrix_paired_shuffle.md`](velocity_matrix_paired_shuffle.md)로 읽는다. "안 무너진다"에서 "무력하다"로 넘어가는 해석은 철회했다(§0 정정).
...
지표는 일치를 잡아낼 수 있다. 같은 method 안에서는 0.87을 준다. 그러니 method 간 0은 측정 한계가 아니라 **실제 불일치**다.

표의 "ATAC 셔플 +0.838"은 **세포집합이 어긋난 값**이다(천장=재표집 15,315 cell, 셔플=전량 21,878 cell). 여기서 끌어냈던 "셔플해도 재적합 잡음만큼밖에 안 움직인다"는 읽기는 **철회한다.** 같은 재표집 세포집합 S_b에서 다시 맞추면 셔플 값은 +0.784·+0.813·+0.810으로 온전한 재적합 범위(+0.826~+0.887) 아래에 3/3 놓이고 세포 부트스트랩 CI도 3/3 비중첩이다([`velocity_matrix_paired_shuffle.md`](velocity_matrix_paired_shuffle.md)). 짝지은 Δ는 +0.081·+0.004·+0.063(중앙값 +0.063, 천장의 7% 수준)으로 작고 고르지 않으며, 그중 하나는 사실상 null이다. 같은 세포·같은 설정의 run-to-run 귀무가 없어 이 Δ는 크로마틴 기여와 재적합 잡음이 섞인 값이므로, 크로마틴의 인과 기여를 **양성으로 단정하지 않는다.** 재현성 천장 자체(+0.872)는 이 정정의 영향을 받지 않는다.
...
2. **크로마틴을 파괴해도 행렬은 붕괴하지 않는다. 다만 "무력하다"는 결론은 철회했다 — MultiVelo 한정.** 짝맞춘 대조에서 셔플 값은 온전한 재적합 범위 아래로 3/3 내려가므로(+0.784·+0.813·+0.810 대 +0.826~+0.887, CI 3/3 비중첩) 크로마틴이 무력하다고 더는 단정할 수 없다. 그러나 짝지은 Δ는 +0.081·+0.004·+0.063으로 작고 고르지 않고(하나는 사실상 null), 같은 지표에서 method 선택이 만드는 불일치(−0.530~+0.131)보다 한참 작다. 같은 세포·같은 설정 run-to-run 귀무가 없어 이 Δ에는 재적합 잡음이 섞여 있으므로 **양성 인과 주장도 하지 않는다.** MoFlow는 재실행 대조가 없어 이 논의에서 제외한다(§6).
...
- ~~**천장과 ATAC-shuffle은 서로 다른 섭동이다**(재적합 15,315 cell, 셔플 전량 21,878 cell)이므로 천장은 하한이고 비교는 "크로마틴 무력" 결론에 관대하다~~ → **해소·정정.** 세포집합을 맞춘 짝맞춘 대조를 실행했고([`velocity_matrix_paired_shuffle.md`](velocity_matrix_paired_shuffle.md)), 그 결과 "무력" 결론을 철회했다(§0). 남는 한계는 다른 것이다. **같은 세포·같은 설정의 run-to-run 귀무가 아직 없어** 짝지은 Δ는 크로마틴 기여와 재적합 잡음의 합이며, 짝맞춘 B arm은 3회뿐이다.
