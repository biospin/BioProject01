...
- **층③(임베딩 화살표)은 아직 안 봤다.** 단, "평활되면 겉보기 궤적은 비슷해질 것"이라는 도피구는 쓸 수 없다 — 게이트 확인 결과([`NOTE_benchmarks_12_13_scope_check.md`](../manuscript/NOTE_benchmarks_12_13_scope_check.md)) [12]는 이웃 평활을 거친 **전이 벡터 수준에서도 방법 간 일치가 낮다**고 보고한다(A1<0.3, 12개 방법 전부). 즉 불일치는 위층에서 회복되지 않는다. 남는 여지는 **거친 흐름선이 눈으로 비슷해 보일 수 있다**는 정성적 수준까지다. 층③의 정량 감사는 여전히 별도 측정 대상이다.
- ~~HSPC 단일 시스템~~ → **외부 재현 완료**(BIOP01-60, `velocity_matrix_audit_external.md`): 기준선 순서(Δ<0)가 외부 4개 시스템에서 4/4 재현(원척도·중심화 둘 다). 단 **재현된 것은 순서뿐** — 인과 대조와 재현성 천장은 외부에 arm이 없어 재현 불가이고, "모든 multiome 쌍이 귀무 이하"는 gastrulation에서 깨진다(MultiVelo×VAE 원척도 +0.200 > 귀무 +0.038).
- **사전등록 측정축 (2)와 유전자 셔플 null은 최초 실행에서 빠졌다가 자체 critic 지적으로 추가 실행했다**(2026-07-19). 결과: multiome 쌍 중 유전자 셔플 귀무를 넘은 것은 MultiVelo×MultiVeloVAE 하나뿐(초과분 +0.328)이고, 전체 최대 초과분 둘은 다시 RNA-only와의 쌍(+0.380·+0.278)이다. 판정은 세포축 대조 A·B로 내리므로 **판정 불변**.
- 부호 일치율의 0(방향 미정) 제외 비율은 쌍마다 0~30.5%다. 50%는 무정보 기준선이 아니므로 같은 데이터의 RNA-only 쌍(51.6~64.3%)에 견주어 읽는다.
- **천장과 ATAC-shuffle은 서로 다른 섭동이다** — 재적합은 21,878개 중 15,315개 세포로, 셔플은 전량으로 계산된다. 재표집은 코사인을 낮추기만 하므로 천장은 **하한**이고, 이 비교는 "크로마틴 무력" 결론에 관대한 쪽이다.
...
- ✅ **선결 게이트 해소**([`NOTE_benchmarks_12_13_scope_check.md`](../manuscript/NOTE_benchmarks_12_13_scope_check.md)): [12,13] 둘 다 임베딩·전이 벡터 수준 채점이고 행렬의 method 간 비교도 인과 chromatin 대조도 없다. 중복·모순 아님. **draft 편입 완료**(커밋 7e42be4·01fa80a), 이후 자체 critic 재검토 반영(REVIEW-GB-2026-07-19b).
