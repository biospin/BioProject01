...
| **③ GSE205117(mouse gastrulation) heavy-run 재개** | 🔄 GEX **2/4 완료** (E7.5·E8.0 ✅, E8.5 진행, E8.75 대기) | 다운·변환 DONE(8런). 드라이버 **PID 8400(PPID=1)**. E8.0은 재기동 후 ~2h 완료(I/O 개선). 로그 `/home/kkkim/data/gse205117_fullB/{gex_solo.log,star_gex.log}`, 진행 `GEX_SOLO_PROGRESS`, 완료=`GEX_SOLO_DONE`. **⚠️ watchdog 부착**(PID 69195, PPID=1, flock 단일락, `watchdog.log`) — 드라이버 조용한 사망(13:10 사례) 재발 시 stale STARtmp 정리 후 자동 재기동(최대 6회). **⚠️ 디스크 84%(2.2T 여유)**: fastq 3.6T 중 `_3/_4`(10x index read, STAR 미사용) 런당 ~300GB 잉여 — GEX엔 충분하나 ATAC 4런+build 전에 정리 검토(kkkim 결정, 미삭제). |
