...
| **③ GSE205117(mouse gastrulation) 5번째 cross-dataset** | ✅ **완료 — 봉인 6예측 6/0 PASS**(결정론 재계산 검증, 커밋 cfecc5d) | scorecard `results/prereg_gse205117_scorecard.md`: α within median +0.927 / lag -0.026 / Δρ +0.979 / cross α +0.415>lag +0.028 / per-gene 불일치 lag 0.307>α 0.052. **priming 극대에서도 α-robust/lag-fragile 재현.** 10779 cells. 예측5=사전선언 MV-vs-VAE 치환(MoFlow arm 부재). 채점기 버그수정 a34c10d(예측5 조용한 VAE 폴백=봉인위반 차단). FINDINGS §7 병합은 사람 검토 후. |
| ~~③-이전~~ GEX→ATAC 파이프라인 이력 | ✅ | GEX 4/4 완료 → ATAC=GEO fragments(raw 정렬 불필요) → GEX 4런 Velocyto matrix 확보(E7.5 8595 / E8.0 6587 / E8.5 8596 / E8.75 3586, u/s 0.33–0.45). watchdog 자동종료. **★ 전환점(2026-07-14): GEO가 processed ATAC `fragments.tsv.gz`를 sample별 제공**(E7.5=GSM6205427·E8.0=GSM6205430·E8.5=GSM6205434·E8.75=GSM6205436, 각 2.4–3.3GB) → **raw fastq 정렬·cellranger-arc/chromap 불필요**(툴 서버 부재였음), human_brain 경로(자체 peak→gene 집계)로 cheap 진행. 분석 단계: fragments 다운→build(atac_gene, 10x ARC 바코드 매칭)→floor+MultiVelo+VAE fit(cuda:1)→P3 concordance→`p3_prereg_gse205117.py` 6예측 대조(사후구제 금지). |
| **⑤ 디스크 raw 정리** | ✅ 실행 완료 (kkkim 승인) | **(1)** skin(NO-GO)+preflight 잔재+BMMC BAM ~200G 삭제(파생 보존본 가드 통과). **(2)** `reclaim_gse205117_raw.sh --execute` — GEX index read(_3/_4, STAR 미사용) 8파일 **~790G 삭제**. 디스크 **85%→78%(3.1T 여유)**. 전체 raw(sra 177G+fastq 2.8T)는 게이트(ATAC·processed h5ad 미완)가 막아 **보존**. 재현=공개 SRA 재다운(sha256). |
| **④ STEP2(GSE70677 크로마틴) 분석** | ✅ 완료(exploratory) `410cda6` | A1 α R²=0.337/0.365(유의) vs A2 lag 0.017/−0.038(비예측) = 후성유전 baseline 축에서 α-robust/lag-fragile 재확인. A4 tempering: 히스톤 incremental ΔR²=0.016(작음)→ '히스톤이 α설명'이 아니라 'α는 baseline-예측가능'. 산출 `results/step2/`, **FINDINGS/PAPER-PLAN 미접촉**(팀 게이트 전). 한계: input/CAGE supp NONE·baseline_expr 대체·H3K27me3 부재. |
