...
def ensure_image_dependencies() -> None:
    global Image, ImageDraw, fitz, np
    if Image is not None and ImageDraw is not None and fitz is not None and np is not None:
        return
    try:
        ...
        print(
            "Missing dependency. Install with: python3 -m pip install pymupdf pillow numpy",
            file=sys.stderr,
        )
...
def gray_array(image: Image.Image) -> Any:
    """8-bit grayscale view of a PIL image as an (H, W) numpy array (row=y, col=x)."""
...
def ink_bbox(gray: Any, threshold: int = 245) -> Box:
    ...
    return Box(int(xs[0]), int(ys[0]), int(xs[-1]) + 1, int(ys[-1]) + 1)


def projection(gray: Any, box: Box, threshold: int, axis: str) -> list[int]:
    sub = gray[box.y0 : box.y1, box.x0 : box.x1] < threshold
    # axis "x" → ink count per column (one entry per x); "y" → per row (one entry per y).
    counts = sub.sum(axis=0) if axis == "x" else sub.sum(axis=1)
...
def best_split(gray: Any, box: Box, threshold: int, min_gutter: int) -> tuple[str, int] | None:
    candidates: list[tuple[int, str, int]] = []
    for axis in ("x", "y"):
        counts = projection(gray, box, threshold, axis)
    ...
    while changed and len(pending) < max_panels:
        ...
        for current in pending:
            if current.width < min_panel * 2 and current.height < min_panel * 2:
                ...
                continue
            split = best_split(gray, current, threshold, min_gutter)
            if split is None:
                next_boxes.append(current)
                continue
            axis, pos = split
...
def main() -> int:
    ...
    figure_gray = gray_array(figure_image)
    ...
    if spec.get("panels"):
        ...
        content = ink_bbox(figure_gray, args.threshold).pad(args.padding, *figure_image.size)
