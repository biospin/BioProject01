...
핵심 산출물 두 가지:
1. `analysis/<primary-topic>/<paper-id>/lens-industry.md` — 4개 섹션의 해석 노트.
...
`lens-industry.md`는 다음 4개 섹션을 순서대로 갖는다.
...
본문 분석 결과(core.md, lens-academic.md)와 paper-info.yaml의 메타데이터를 종합해 다음 6개 중 1~3개 선택:

| Tag | 적용 시점 |
|---|---|
| `academic-citation` | 본인 논문·제안서·학회 발표에서 인용 가치 있을 때. `lens-academic.md`의 Citation 후보 섹션이 풍부하면 자동 부여. |
...
Section 1.3의 Level과 Perspective를 *3~5 bullet으로 풀어 설명*. paper-info.yaml에는 1문장만 있고, lens-industry.md에서는 자세한 근거.

```markdown
- Level: 상
- Perspective: HSPC multiome 파이프라인에 바로 차용 가능 + chromatin-RNA lag 알고리즘이 우리 핵심 문제 직결.
- 등급 근거:
  - core.md §3.2의 dataset (HSPC 10x Multiome, n=24k cells)이 우리 현재 dataset과 동일 platform.
