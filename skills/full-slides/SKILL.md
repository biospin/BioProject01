---
name: full-slides
description: Create static journal-meeting slide decks from existing topic-organized paper analyses. Use only when the user explicitly asks to make slides, a slide deck, or a presentation from an analyzed paper. Requires an existing analysis/<primary-topic>/<paper-id>/core.md, uses design.md as the required visual design reference, follows the installed openclaw-slides skill for browser presentation structure, and should include captured figure images from the source PDF when explaining figures.
---

# Full Slides

## 목표
이미 생성된 `analysis/<primary-topic>/<paper-id>/core.md`를 기준으로 journal meeting용 static slide deck을 만든다. 이 스킬은 자료 분석 과정에서 매번 자동 실행하지 않는다. 사용자가 "슬라이드 만들어줘", "발표자료 만들어줘", "presentation 만들어줘"처럼 명시적으로 요청했을 때만 사용한다.

## Source grounding
- Source grounding 원칙은 `skills/source-grounding/SKILL.md`를 따른다. PDF Figure 캡처 등은 반드시 `sources/` 안의 파일만 사용한다.
- slide 본문은 `core.md`와 `lens-*.md` 내용에 근거한다. core.md에 없는 새 주장을 slide에 만들지 않는다.

## 필수 전제
- `analysis/<primary-topic>/<paper-id>/core.md`가 먼저 존재해야 한다.
- `core.md`가 없으면 slide를 만들지 말고 먼저 full paper analysis가 필요하다고 말한다.
- source PDF가 `papers/` 또는 `core.md`의 Source PDF 경로에 있어야 Figure 이미지를 캡처할 수 있다.
...
- slide 한 장에 너무 많은 문장을 넣지는 않되, `core.md`에 정리된 배경, 비교, 수치, limitation을 충분히 반영한다.
...
## 입력
- 특정 topic과 논문이 지정되면 해당 `analysis/<primary-topic>/<paper-id>/core.md`를 사용한다.
- 자료만 지정되고 topic이 지정되지 않으면 `analysis/**/<paper-id>/core.md`를 검색해 가장 일치하는 파일을 사용한다.
- topic만 지정되고 그 아래 paper folder가 하나뿐이면 그 `core.md`를 사용한다.
...
## Slide 구성 원칙
- `core.md`를 그대로 옮기지 말고 journal meeting에서 설명 가능한 주제별 slide로 나눈다.
- 슬라이드 수는 제한하지 않는다. 내용을 충분히 설명하기 위해 필요하면 Figure별, panel별로 여러 장을 만든다.
- 단순 요약 slide보다 `core.md`의 설명 구조를 충실히 보이도록 한다.
...
- 수치, dataset, baseline, metric은 `core.md`에 있는 내용만 사용한다.
...
## PDF Figure 캡처 절차
- 먼저 source PDF 경로를 확인한다. `core.md`에 `Source PDF`가 있으면 그 경로를 우선한다.
...
## 작업 절차
1. `analysis/<primary-topic>/<paper-id>/core.md` 존재 여부를 확인한다.
2. `design.md`를 읽는다.
...
6. `core.md`에서 slide topics를 뽑는다.
...
## 금지 사항
- `core.md`가 없는 상태에서 slide 내용을 만들어내지 않는다.
- PDF 원문이나 외부 지식을 근거로 slide 내용을 확장하지 않는다. 필요한 경우 먼저 `core.md`를 업데이트한다.
