...
└── figures/
    ...
    ├── ...
    ├── figure-map.json              # 페이지+bbox 매핑 (수정 가능, 5.2절)
...
4. 각 Figure 페이지에서 *caption 끝까지의 bbox*만 잘라 `figures/figure-N.png`로 export
   (한 페이지에 figure + body text가 섞여 있어도 본문이 들어가지 않음 — 5.3절 참고).
...
8. `<head>`에 MathJax v3 CDN + config 주입. `$...$`(인라인), `$$...$$`(블록),
   `\(...\)`, `\[...\]` 모두 브라우저에서 자동 렌더 (5.4절).
9. `<paper-id>_core.html` 저장.
...
### 5.2 Figure 자동 crop (smart bbox)

기본 page-level 모드는 figure 페이지 *전체*를 PNG로 자르는 대신, *figure + caption*까지만 잘라낸다.

- caption 시작은 `^\s*Fig(?:ure)?\.?\s*N\s*[|.]` 패턴으로 검출.
- caption block이 끝나는 Y 좌표 + 6pt 까지를 bbox 하한으로 사용. 페이지 하단과 50pt 이내면 그냥 page bottom.
- caption regex가 매칭 안 되면 *경고를 출력*하고 whole-page snapshot으로 fallback.
- 결과 bbox는 `figures/figure-map.json`에 `{ "figure-N": { "page": P, "bbox": [x0,y0,x1,y1] } }` schema로 저장.
- 구형 schema(`{ "figure-N": P }` 정수 값)도 그대로 읽을 수 있다 — 다음 build에서 새 schema로 변환.

이 동작으로 *한 페이지에 figure + 본문 단락이 섞여 있는* 경우(예: 첫 페이지 Figure 1) 본문이 PNG에 포함되지 않는다.

### 5.3 MathJax 수식 렌더링

HTML head에 MathJax v3 CDN과 config가 자동 주입된다. core.md / lens-*.md 등에 다음 delimiter로 적은 수식이 브라우저에서 자동 렌더된다:

| 종류 | Delimiter | 예시 |
|---|---|---|
| inline | `$...$` 또는 `\(...\)` | `$\alpha_c$` |
| block | `$$...$$` 또는 `\[...\]` | `$$\frac{d s}{d t} = \alpha_c\,c - \beta s$$` |

markdown 파싱 직전에 math와 code span을 placeholder로 빼두고 HTML 생성 후 복원하기 때문에 수식 안의 `_`, `*`, `\` 가 italic/bold/escape로 깨지지 않는다.

코드 펜스(```` ``` ````)와 inline code 안에 적힌 `$`는 *항상 코드 그대로* 보이고 MathJax가 건드리지 않는다.
