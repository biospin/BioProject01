---
name: full-results
description: Analyze the Results section of a full scientific paper review. Use when Codex needs to summarize which datasets were used, what experimental settings or baselines were compared, and what concrete numerical or qualitative results were obtained for each dataset.
---

# Full Results

## 목표
Results 섹션을 “어떤 dataset을 사용했고, 그 dataset에서 어떤 결과값을 얻었는가” 중심으로 정리한다. 핵심은 논문의 주장을 뒷받침하는 실제 수치, dataset 이름, sample size, gene 수, cell 수, baseline, metric, 비교 결과를 빠뜨리지 않는 것이다.

## 언어 규칙
AGENTS.md의 언어 규칙을 따른다.

## 작업 절차
0. `full.md`의 현재 내용을 읽는다. Background, Overview, Methods(작성된 경우)에서 정리된 dataset 목록과 주요 주장을 파악한 뒤 Results 작성을 시작한다.
