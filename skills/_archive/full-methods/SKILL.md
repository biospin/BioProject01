...
## 언어 규칙
AGENTS.md의 언어 규칙을 따른다. 수식이 있으면 그대로 보존하되, 각 term의 역할을 한국어로 설명한다. `Markov`, `gradient`, `negative sampling`, `supervised`, `unsupervised`, `semisupervised`처럼 AGENTS.md 목록에 없는 ML 용어도 영어로 유지할 수 있다.
