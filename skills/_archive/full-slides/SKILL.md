...
## 언어 규칙
AGENTS.md의 언어 규칙을 따른다. slide에 추가로 적용하는 규칙:
...
- 먼저 `/Users/jamie/.openclaw/workspace/skills/openclaw-slides/SKILL.md`가 있는지 확인한다.
- OpenClaw Slides가 없으면 slide를 만들기 전에 `openclaw skills install openclaw-slides`로 다운로드해야 한다고 말하고, 가능하면 해당 명령을 실행해 설치한다.
- 설치 후에는 `/Users/jamie/.openclaw/workspace/skills/openclaw-slides/SKILL.md`를 필요한 만큼 읽고 그 구조를 따른다.
